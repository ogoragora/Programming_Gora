    ...
    for name in set1 & set2:
        ...
        if os.path.isfile(fullname1) and os.path.isfile(fullname2):
            ...
            if ctime1 < ctime2:
                ...
                print(
                    f"Файл {name} ({date1}) з каталогу {dir1} було створено раніше за файл {name} ({date2}) з каталогу {dir2}."
                )
            elif ctime1 > ctime2:
                date1 = time.ctime(ctime1)
                date2 = time.ctime(ctime2)
                print(
                    f"Файл {name} ({date1}) з каталогу {dir1} було створено пізніше за файл {name} ({date2}) з каталогу {dir2}."
                )
            else:
                print(f"Файли {name} мають однаковий час створення")
